-- Comprehensive RLS Setup Script for EJA Homestay
-- This script enables RLS and sets policies for all tables
...
DROP POLICY IF EXISTS "Experiences can be updated by creator" ON experiences;
DROP POLICY IF EXISTS "Experiences can be deleted by creator" ON experiences;
...
-- Policy to allow creators to update their own experiences
CREATE POLICY "Experiences can be updated by creator" ON experiences
  FOR UPDATE USING (auth.uid() = created_by);

-- Policy to allow creators to delete their own experiences
CREATE POLICY "Experiences can be deleted by creator" ON experiences
...
DROP POLICY IF EXISTS "Trips can be deleted by creator" ON trips;
...
-- Policy to allow creators to update their own trips
CREATE POLICY "Trips can be updated by creator" ON trips
  FOR UPDATE USING (auth.uid() = created_by);

-- Policy to allow creators to delete their own trips
