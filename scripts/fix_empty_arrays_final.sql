-- Fix malformed array literal error for empty arrays
...
-- Step 2: Recreate the view with proper empty array handling
...
SELECT 
    ...
    city as location,
    CASE 
        WHEN images IS NULL OR array_length(images::text[], 1) IS NULL THEN ARRAY[]::text[]
        ELSE images::text[]
    END as images,
...
SELECT type, title, array_length(images, 1) as image_count FROM public.unified_search_view LIMIT 5;
