...
export default function TripDetailPage() {
  ...
  useEffect(() => {
    const fetchTrip = async () => {
      const { data } = await supabase.from("trips").select("*, reviews:trip_reviews(*)").eq("id", tripId).single();
      setTrip(data);
  ...
  useEffect(() => {
    // TODO: Check if user has completed booking for this trip
    setCanReview(!!user);
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    const { data, error } = await supabase.from("trip_reviews").insert({
      trip_id: tripId,
      guest_id: profile.id,
      rating: reviewRating,
      comment: reviewText,
    ...
    <div className="max-w-3xl mx-auto py-8">
      ...
      <Card>
        <CardContent className="p-6">
          ...
          {hasReviewed && user && (
            <div className="text-center text-green-600 mt-8">You have already reviewed this trip. Thank you!</div>
          )}
        </CardContent>
      </Card>
    </div>
